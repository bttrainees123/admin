...
const HomePage = () => {
  ...
  const uploadBase64 = async(imageUrl, txt) => {
    try{
      const response = await axios.post(`https://n-again.com/api/uploadbase64`, imageUrl)
      if(response){
        console.log('response ',response);
        setImgData((prev) => [...prev, { image: imageUrl, text: txt, path: 'https://n-again.com/images/' + response.data.path }])
